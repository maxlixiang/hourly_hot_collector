...
RSS 采集是增量采集：每轮会以上一次 `success` 或 `partial` 采集记录的 `finished_at` 作为窗口起点；如果还没有上一次成功记录，则回退到当前时间前 1 小时。只有 `pubDate` / `published` / `updated` 晚于窗口起点且不晚于本轮运行时间的 RSS item 会被采集。
...
热点查询支持三个返回阶段领域过滤：`finance`（财经）、`geopolitics`（地缘政治）、`tech_ai`（AI/科技）。planner 的用户意图触发词保持收敛：`财经` -> `finance`；`政治`、`国际`、`地缘`、`地缘政治` -> `geopolitics`；`科技`、`AI`、`ai` -> `tech_ai`。领域过滤不影响采集和聚类，程序仍先生成全量热点，再根据标题、摘要、来源和 cluster 文章标题做规则筛选。也可以显式传参：`--domain finance|geopolitics|tech_ai`。
